...
import { validateToolParameters, validateCampaignData, validateCampaignPrerequisiteData, validateGetCampaignAnalyticsData } from './validation.js';
    ...
    try {
        ...
        switch (name) {
                ...
                switch (stage) {
                    ...
                    case 'create': {
                        ...
                        if (!args?.confirm_creation) {
                            throw new McpError(ErrorCode.InvalidParams, 'Campaign creation requires explicit confirmation. Set confirm_creation=true to proceed. ' +
                        ...
                        if (missingFields.length > 0) {
                            throw new McpError(ErrorCode.InvalidParams, `Missing required fields for campaign creation: ${missingFields.join(', ')}. ` +
                    ...
                    default:
                        throw new McpError(ErrorCode.InvalidParams, `Invalid stage: ${stage}. Valid stages are: prerequisite_check, preview, create`);
                }
            }
            ...
            case 'get_campaign_analytics': {
                try {
                    ...
                    if (error.message?.includes('404') && args?.campaign_id) {
                        ...
                        throw new McpError(ErrorCode.InvalidParams, `Campaign analytics not found for campaign_id: ${args.campaign_id}. ` +
                        ...
                        throw new McpError(ErrorCode.InvalidRequest, `Campaign analytics endpoint not available. The Instantly API may not support analytics for your account type.`);
                    }
            ...
            case 'list_accounts': {
                ...
                if (wantsAllAccounts) {
                    ...
                    try {
                        ...
                        const enhancedResult = {
                            ...
                            campaign_creation_guidance: {
                                ...
                                verified_accounts: allAccounts.filter((account) => account.status === 1 && !account.setup_pending && account.warmup_status === 1).map((account) => account.email),
                                total_accounts: allAccounts.length,
                                verified_count: allAccounts.filter((account) => account.status === 1 && !account.setup_pending && account.warmup_status === 1).length,
                    ...
                    const enhancedResult = {
                        ...
                        campaign_creation_guidance: {
                            message: "Use the email addresses from the 'data' array above for campaign creation",
                            verified_accounts: result.data?.filter((account) => account.status === 1 && !account.setup_pending && account.warmup_status === 1).map((account) => account.email) || [],
            ...
            case 'list_leads': {
                ...
                if (args?.campaign_id)
                    requestData.campaign_id = args.campaign_id;
                if (args?.list_id)
                    requestData.list_id = args.list_id;
                if (args?.status)
                    requestData.status = args.status;
            ...
            case 'create_lead': {
                ...
                if (args.firstName)
                    leadData.first_name = args.firstName;
                if (args.lastName)
                    leadData.last_name = args.lastName;
                if (args.companyName)
                    leadData.company_name = args.companyName;
                if (args.website)
                    leadData.website = args.website;
                if (args.personalization)
                    leadData.personalization = args.personalization;
                if (args.custom_fields)
                    leadData.custom_fields = args.custom_fields;
            ...
            case 'reply_to_email': {
                ...
                if (!args.reply_to_uuid || args.reply_to_uuid === 'test-uuid') {
                    throw new McpError(ErrorCode.InvalidParams, `reply_to_uuid must be a valid email ID. Use list_emails or get_email tools first to obtain a valid email UUID.`);
                }
                // Validate body object has either html or text
                const bodyObj = args.body;
            ...
            case 'verify_email': {
                ...
                if (!availabilityCheck.available) {
                    throw new McpError(ErrorCode.InvalidRequest, `Email verification is not available: ${availabilityCheck.reason} ` +
                ...
                try {
                    ...
                    if (error.message?.includes('403') || error.message?.includes('Forbidden')) {
                        throw new McpError(ErrorCode.InvalidRequest, `Email verification access denied (403): This feature requires a premium Instantly plan. ` +
                    ...
                    if (error.message?.includes('401') || error.message?.includes('Unauthorized')) {
                        throw new McpError(ErrorCode.InvalidRequest, `Email verification unauthorized (401): Your API key may be invalid or expired. ` +
                            `Please verify your API key is correct and has the required permissions.`);
                    }
                    if (error.message?.includes('429') || error.message?.includes('Too Many Requests')) {
                        throw new McpError(ErrorCode.InvalidRequest, `Email verification rate limited (429): Too many verification requests. ` +
                    ...
                    if (error.message?.includes('402') || error.message?.includes('Payment Required')) {
                        throw new McpError(ErrorCode.InvalidRequest, `Email verification requires payment (402): This feature is not included in your current plan. ` +
            ...
            case 'validate_campaign_accounts': {
                try {
                    ...
                    for (const account of accounts) {
                        ...
                        if (account.status !== 1)
                            accountInfo.issues.push('Account not active (status != 1)');
                        if (account.setup_pending)
                            accountInfo.issues.push('Setup still pending');
                        if (account.warmup_status !== 1)
                            accountInfo.issues.push('Warmup not active (warmup_status != 1)');
                        if (!account.email)
                            accountInfo.issues.push('No email address');
            ...
            case 'get_account_details': {
                ...
                try {
                    ...
                    const account = accounts.find((acc) => acc.email.toLowerCase() === args.email.toLowerCase());
                    if (!account) {
