...
class VoiceInfo(BaseModel):
    ...
    gender: str
    name: str
    preview_audio: Optional[str] = None  # Allow empty strings or None
    support_pause: bool
    emotion_support: bool
    support_interactive_avatar: bool
    
    @field_validator('preview_audio', mode='before')
    @classmethod
    def validate_preview_audio(cls, v):
        # Convert empty string to None for optional field
        if v == "" or v is None:
            return None
        # Return the URL as is if it's a valid string
        return v
...
class RemainingQuota(BaseModel):
    ...
    # Handle case where API returns quota details directly
...
class HeyGenApiClient:
    ...
    async def get_remaining_credits(self) -> MCPGetCreditsResponse:
        ...
        def transform_data(data, mcp_class):
            # Handle case where API returns remaining_quota field
            if hasattr(data, 'remaining_quota') and data.remaining_quota is not None:
                return mcp_class(remaining_credits=int(data.remaining_quota / 60))
            # Handle case where API returns quota details directly (api field)
            elif hasattr(data, 'api') and data.api is not None:
                return mcp_class(remaining_credits=data.api)
            else:
                return mcp_class(remaining_credits=0)
