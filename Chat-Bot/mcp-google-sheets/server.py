...
ACCESS_TOKEN = os.environ.get('GSHEETS_ACCESS_TOKEN')
REFRESH_TOKEN = os.environ.get('GSHEETS_REFRESH_TOKEN')
...
async def spreadsheet_lifespan(server: FastMCP) -> AsyncIterator[SpreadsheetContext]:
    ...
    if not ACCESS_TOKEN or not REFRESH_TOKEN:
        raise Exception("GSHEETS_ACCESS_TOKEN and GSHEETS_REFRESH_TOKEN environment variables are required")

    # Create credentials from tokens (similar to Gmail server)
    creds = Credentials(
        token=ACCESS_TOKEN,
...
def get_sheet_data(spreadsheet_id: str, 
    ...
    Returns only non-empty cell values in a lightweight format to minimize token usage.
    ...
    Returns:
        Lightweight data structure containing only non-empty cell values
    """
    try:
        ...
        # Use values API for lightweight data retrieval (no formatting metadata)
        ...
        def column_number_to_letter(column_num):
            """Convert 0-based column number to Excel-style column letter"""
            result = ""
            while column_num >= 0:
        ...
        # Filter out empty rows and convert to a clean structure with column letters
        non_empty_data = []
        for row_index, row in enumerate(values):
            ...
            # For non-empty rows, create a dictionary with column letters as keys
            row_data = {}
            for col_index, cell in enumerate(row):
                column_letter = column_number_to_letter(col_index)
                row_data[column_letter] = str(cell) if cell else ""
            
            clean_row = {
                "row": row_index + 1,  # 1-based row numbering
                "values": row_data
            }
            non_empty_data.append(clean_row)
...
def get_sheet_formulas(spreadsheet_id: str,
                       sheet: str,
                       range: Optional[str] = None,
                       ctx: Context = None) -> Union[List[List[Any]], Dict[str, str]]:
    """
    ...
    Returns:
        A 2D array of the sheet formulas or error dict.
    """
    try:
        # Validate required parameters
        if not spreadsheet_id:
        ...
        return {"error": f"Failed to get sheet formulas: {str(e)}"}
...
def update_cells(spreadsheet_id: str,
    ...
    try:
        ...
        return {"error": f"Failed to update cells: {str(e)}"}
...
def batch_update_cells(spreadsheet_id: str,
    ...
    try:
        ...
        if not sheet:
            return {"error": "sheet name is required"}
        if not ranges:
            return {"error": "ranges dictionary is required"}
        if not isinstance(ranges, dict):
            return {"error": "ranges must be a dictionary mapping range strings to 2D arrays"}
        ...
        for range_str, values in ranges.items():
            if not isinstance(values, list):
                return {"error": f"Values for range '{range_str}' must be a 2D array (list of lists)"}
        ...
        return {"error": f"Failed to batch update cells: {str(e)}"}
...
def add_rows(spreadsheet_id: str,
    ...
    try:
        ...
        if start_row is not None and (not isinstance(start_row, int) or start_row < 0):
            return {"error": "start_row must be a non-negative integer"}
        ...
        return {"error": f"Failed to add rows: {str(e)}"}
...
def add_columns(spreadsheet_id: str,
    ...
    try:
        ...
        if not isinstance(count, int) or count <= 0:
            return {"error": "count must be a positive integer"}
        if start_column is not None and (not isinstance(start_column, int) or start_column < 0):
            return {"error": "start_column must be a non-negative integer"}
        ...
        return {"error": f"Failed to add columns: {str(e)}"}


@mcp.tool()
def list_sheets(spreadsheet_id: str, ctx: Context = None) -> Union[List[str], Dict[str, str]]:
    """
    ...
    Returns:
        List of sheet names or error dict
    """
    try:
        # Validate required parameters
        if not spreadsheet_id:
...
def copy_sheet(src_spreadsheet: str,
    ...
    try:
        ...
        if not src_spreadsheet:
            return {"error": "src_spreadsheet ID is required"}
        if not src_sheet:
            return {"error": "src_sheet name is required"}
        if not dst_spreadsheet:
            return {"error": "dst_spreadsheet ID is required"}
        if not dst_sheet:
            return {"error": "dst_sheet name is required"}
        ...
        return {"error": f"Failed to copy sheet: {str(e)}"}
...
def get_spreadsheet_info(spreadsheet_id: str) -> str:
    ...
    try:
        ...
        return json.dumps({"error": f"Failed to get spreadsheet info: {str(e)}"}, indent=2)
...
def create_spreadsheet(title: str, ctx: Context = None) -> Dict[str, Any]:
    ...
    try:
        ...
        # Spreadsheet will remain in My Drive root (no folder organization)
...
def list_spreadsheets(ctx: Context = None) -> List[Dict[str, str]]:
    ...
    try:
        ...
        # Always search in 'My Drive' (all accessible spreadsheets)
...
@mcp.tool()
def append_row(spreadsheet_id: str,
               sheet: str,
               columns: List[str],
               data: List[Any],
               ctx: Context = None) -> Dict[str, Any]:
    """
    Append data to the first empty row in a Google Spreadsheet, writing to specified columns.
    
    Args:
        spreadsheet_id: The ID of the spreadsheet (found in the URL)
        sheet: The name of the sheet
        columns: List of column letters to write to (e.g., ['A', 'B', 'D'])
        data: List of values to write, corresponding to the columns array
    
    Returns:
        Result of the operation including the row number where data was written
    """
    try:
        # Validate required parameters
        if not spreadsheet_id:
        ...
        if not sheet:
            return {"error": "sheet name is required"}
        if not columns:
            return {"error": "columns array is required"}
        if not data:
            return {"error": "data array is required"}
        if not isinstance(columns, list):
            return {"error": "columns must be a list of column letters"}
        if not isinstance(data, list):
            return {"error": "data must be a list of values"}
        if len(columns) != len(data):
            return {"error": "columns and data arrays must have the same length"}
        
        # Validate column letters
        for col in columns:
            if not isinstance(col, str) or not col.isalpha():
                return {"error": f"Invalid column '{col}'. Columns must be letters (A, B, C, etc.)"}
        
        sheets_service = ctx.request_context.lifespan_context.sheets_service
        
        # Get all current data to find the first empty row
        result = sheets_service.spreadsheets().values().get(
            spreadsheetId=spreadsheet_id,
            range=sheet,
        ...
        first_empty_row = 1  # Default to row 1 if sheet is empty
        for row_index, row in enumerate(values):
            # Check if this row is completely empty
            if not any(cell for cell in row if str(cell).strip()):
                first_empty_row = row_index + 1  # Convert to 1-based indexing
                break
        else:
            # If no empty row found, append after the last row
            first_empty_row = len(values) + 1
        
        # Prepare batch update data for each column-value pair
        update_data = []
        for col, value in zip(columns, data):
            cell_range = f"{sheet}!{col}{first_empty_row}"
            update_data.append({
                'range': cell_range,
                'values': [[str(value)]]
            })
        ...
        return {
            "success": True,
            "row_number": first_empty_row,
            "columns_written": columns,
            "values_written": data,
            "updated_cells": result.get('totalUpdatedCells', 0),
            "updated_ranges": [f"{sheet}!{col}{first_empty_row}" for col in columns]
        }
        
    except Exception as e:
        return {"error": f"Failed to append row: {str(e)}"}
