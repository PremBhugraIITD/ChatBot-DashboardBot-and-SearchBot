...
function validateEnvironmentVariables() {
    const requiredVars = ['GDOCS_ACCESS_TOKEN', 'GDOCS_REFRESH_TOKEN'];
...
async function initializeGoogleClient() {
    ...
    if (!authClient) { // Check authClient instead of googleDocs to allow re-attempt
        try {
            ...
            console.error("Attempting to authorize Google API client with environment tokens...");
            const client = await authorize();
            authClient = client; // Assign client here
            googleDocs = google.docs({ version: 'v1', auth: authClient });
            googleDrive = google.drive({ version: 'v3', auth: authClient });
            console.error("Google API client authorized successfully with environment tokens.");
        }
        catch (error) {
...
const server = new FastMCP({
    name: 'Google Docs MCP Server (Token-Based)',
    version: '1.0.0'
});
...
server.addTool({
    ...
    execute: async (args, { log }) => {
        ...
        try {
            ...
            const request = { insertText: { location: { index: endIndex }, text: textToInsert } };
...
async function startServer() {
    try {
        console.error("🚀 Starting Google Docs MCP Server (Token-Based Authentication)...");
        console.error("📋 Required environment variables: GDOCS_ACCESS_TOKEN, GDOCS_REFRESH_TOKEN");
        await initializeGoogleClient(); // Authorize BEFORE starting listeners
        console.error("✅ Google Docs MCP server initialized successfully with environment tokens");
        ...
        console.error("📊 Available tools: Google Docs & Drive operations (read, write, format, manage)");
        // Log that error handling has been enabled
        console.error('🛡️ Process-level error handling configured to prevent crashes from timeout errors.');
    }
    catch (startError) {
        console.error("❌ FATAL: Server failed to start:", startError.message || startError);
        if (startError.message.includes('environment variables')) {
            console.error("💡 Make sure GDOCS_ACCESS_TOKEN and GDOCS_REFRESH_TOKEN are set in your environment");
        }
        process.exit(1);
    }
}
