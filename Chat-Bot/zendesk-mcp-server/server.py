...
async def handle_call_tool(
    ...
    try:
        ...
        if name == "get_ticket":
            try:
                ticket = zendesk_client.tickets(id=arguments["ticket_id"])
                ticket_data = {
                    'id': ticket.id,
                ...
                return [types.TextContent(
                    type="text",
                    text=json.dumps(ticket_data)
                )]
            except Exception as e:
                raise Exception(f"Failed to get ticket {arguments['ticket_id']}: {str(e)}")

        elif name == "get_ticket_comments":
            try:
                comments = zendesk_client.tickets.comments(ticket=arguments["ticket_id"])
                comments_data = [{
                    'id': comment.id,
                ...
                return [types.TextContent(
                    type="text",
                    text=json.dumps(comments_data)
                )]
            except Exception as e:
                raise Exception(f"Failed to get comments for ticket {arguments['ticket_id']}: {str(e)}")
            ...
            try:
                ...
                ticket.comment = Comment(
                    html_body=arguments["comment"],
                    public=public
                )
                zendesk_client.tickets.update(ticket)
                return [types.TextContent(
                    type="text",
                    text=f"Comment created successfully: {arguments['comment']}"
                )]
            except Exception as e:
                raise Exception(f"Failed to post comment on ticket {arguments['ticket_id']}: {str(e)}")
...
def get_cached_kb():
    try:
        # Get all sections
        sections = zendesk_client.help_center.sections()

        # Get articles for each section
        kb = {}
        for section in sections:
            articles = zendesk_client.help_center.sections.articles(section.id)
            kb[section.name] = {
