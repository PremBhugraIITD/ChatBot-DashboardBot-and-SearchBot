export class IntercomService {
    ...
    ITEMS_PER_PAGE = 150; // Based on Python script maximum
    CONCURRENT_REQUESTS = 5; // Maximum number of concurrent requests
    BATCH_SIZE = 10; // Number of tickets to process in a batch
    PROGRESS_INTERVAL = 10; // Log progress every X tickets
    OPTIMIZATION_THRESHOLD = 50; // Threshold for using optimized processing
     ...
     */
    async makeRequest(endpoint, method = 'GET', params, body, retryCount = 0) {
     ...
     */
    determineMessageSender(authorType, partType) {
        if (partType === 'note') {
            return 'system';
        }
        switch (authorType) {
     ...
     */
    mapIntercomStateToStatus(state) {
        switch (state) {
            case 'open':
                return 'open';
            case 'closed':
     ...
     */
    convertToTickets(conversations, startDate, endDate, keyword, exclude) {
     ...
     */
    async getConversationHistory(ticketId, keyword, exclude) {
        try {
            ...
            const response = await this.makeRequest(`conversations/${ticketId}`, 'GET', params);
            const messages = [];
            ...
            if (error instanceof Error && error.message.includes('404')) {
                ...
                try {
                    const alternateResponse = await this.makeRequest(`admins/conversations/${ticketId}`, 'GET');
     ...
     */
    async addConversationHistories(tickets, keyword, exclude) {
        ...
        const ticketsWithConversations = [];
        ...
        for (let i = 0; i < tickets.length; i += this.BATCH_SIZE) {
            ...
            const batchResults = await this.processTicketBatch(batchTickets, processedCount, total, keyword, exclude);
     ...
     */
    async processTicketBatch(tickets, startIndex, totalTickets, keyword, exclude) {
        const results = [];
        const batches = [];
     ...
     */
    async getAllConversationsWithPagination() {
        ...
        while (morePages) {
            try {
                ...
                const response = await this.makeRequest('conversations', 'GET', params);
     ...
     */
    async getTicketsByStatus(status, startDate, endDate) {
        try {
            ...
            const response = await this.makeRequest('tickets/search', 'POST', undefined, searchQuery);
     ...
     */
    async getConversationsByCustomer(customerIdentifier, startDate, endDate, keywords) {
        try {
            ...
            if (keywords && keywords.length > 0)
                console.error(`Filtering by keywords: ${keywords.join(', ')}`);
            ...
            try {
                ...
                if (isEmail) {
                    ...
                    const searchResponse = await this.makeRequest('contacts/search', 'POST', undefined, searchBody);
     ...
     */
    async getTicketsByCustomer(customerIdentifier, startDate, endDate) {
        try {
     ...
     */
    async searchConversations(startDate, endDate, keyword, exclude, contactId) {
        try {
            ...
            const response = await this.makeRequest('conversations/search', 'POST', undefined, searchQuery);
            if (!response.conversations || response.conversations.length === 0) {
            ...
            const tickets = this.convertToTickets(response.conversations, startDateObj, endDateObj);
            // Get full conversation history for each ticket
     ...
     */
    async getConversations(startDate, endDate, keyword, exclude) {
        try {
            console.error(`Searching for conversations between ${startDate} and ${endDate}...`);
            if (keyword)
                console.error(`Filtering by keyword: "${keyword}"`);
            if (exclude)
                console.error(`Excluding conversations containing: "${exclude}"`);
