import { ListConversationsArgumentsSchema, SearchConversationsByCustomerSchema, SearchTicketsByCustomerSchema, SearchTicketsByStatusSchema } from "../types/schemas.js";
...
export class ToolHandlers {
     ...
     */
    formatErrorResponse(error, customMessage) {
     ...
     */
    async handleSearchConversationsByCustomer(args) {
        try {
            ...
            const conversations = await intercomService.getConversationsByCustomer(customerIdentifier, startDateStr, endDateStr, keywords);
     ...
     */
    async handleSearchTicketsByStatus(args) {
        try {
            ...
            const intercomService = new IntercomService(this.API_BASE_URL, this.authToken);
            const tickets = await intercomService.getTicketsByStatus(status, startDateStr, endDateStr);
     ...
     */
    async handleSearchTicketsByCustomer(args) {
        try {
            ...
            // Create Intercom service and retrieve tickets
            const intercomService = new IntercomService(this.API_BASE_URL, this.authToken);
            const tickets = await intercomService.getTicketsByCustomer(customerIdentifier, startDateStr, endDateStr);
    ...
    async handleListConversations(args) {
        try {
            ...
            // Create Intercom service and retrieve conversations
            const intercomService = new IntercomService(this.API_BASE_URL, this.authToken);
            const conversations = await intercomService.getConversations(startDateStr, endDateStr, keyword, exclude);
