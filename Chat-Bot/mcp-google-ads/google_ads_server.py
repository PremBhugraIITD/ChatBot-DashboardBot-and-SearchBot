...
GOOGLE_ADS_ACCESS_TOKEN = os.environ.get("GADS_ACCESS_TOKEN")
GOOGLE_ADS_REFRESH_TOKEN = os.environ.get("GADS_REFRESH_TOKEN")
...
def get_credentials():
    """
    Get and refresh OAuth credentials using access and refresh tokens.
    Similar to Gmail and Google Sheets servers.
    
    Returns:
        Valid credentials object to use with Google Ads API
    """
    if not GOOGLE_ADS_ACCESS_TOKEN or not GOOGLE_ADS_REFRESH_TOKEN:
        raise ValueError("GADS_ACCESS_TOKEN and GADS_REFRESH_TOKEN environment variables must be set")
    
    logger.info("Creating Google Ads credentials from access/refresh tokens")
    ...
    creds_info = {
        ...
        "client_id": "your_client_id",  # Not used for token refresh
        "client_secret": "your_client_secret",  # Not used for token refresh
        "scopes": SCOPES
    }
    ...
    try:
        ...
        if creds.expired or not creds.valid:
            logger.info("Refreshing expired Google Ads token")
        ...
        logger.error(f"Error creating/refreshing Google Ads credentials: {str(e)}")
        raise
