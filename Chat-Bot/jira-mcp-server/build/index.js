 ...
 * @returns {Promise<any>}
 */
async function addAttachmentFromConfluence(issueIdOrKey, pageId, attachmentName) {
    try {
        ...
        const attachment = response.data.results.find((attachment) => attachment.title === attachmentName);
        if (!attachment) {
            return {
 ...
 * @returns {Promise<any>}
 */
async function addAttachment(issueIdOrKey, imageUrl) {
    try {
 ...
 * @returns {Promise<any>}
 */
async function queryAssignable(project_key) {
    try {
        const params = {
 ...
 * @returns {Promise<any>}
 */
async function executeJQL(jql, maxResults) {
    try {
        const params = {
 ...
 * @param parentID
 */
async function createTicket(project, summary, description, issuetype, parentID) {
    try {
 ...
 * @returns {Promise<any>}
 */
async function deleteTicket(issueIdOrKey) {
    try {
 ...
 * @returns {Promise<any>}
 */
async function editTicket(issueIdOrKey, summary, description, labels, parent) {
    try {
        const descriptionToSend = description || 'No description provided';
        const jiraDescription = description === null ? undefined :
            {
                "type": "doc",
                "version": 1,
                "content": [
 ...
 * @returns {Promise<any>}
 */
async function getAllStatus(number_of_results) {
    try {
        const params = {
            maxResults: number_of_results, // Adjust as needed
        };
 ...
 * @returns {Promise<any>}
 */
async function assignTicket(accountId, issueIdOrKey) {
    try {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    switch (request.params.name) {
        ...
        case "get_only_ticket_name_and_description": {
            ...
            // Check if issues array exists, if not return empty array
            if (!response.issues || !Array.isArray(response.issues)) {
                return {
                    content: [{
                        type: "text",
                        text: JSON.stringify({ message: "No issues found or invalid response format" }, null, 2)
                    }]
                };
            }
        ...
        case 'create_ticket': {
            ...
            const parent = request.params.arguments?.parent;
        ...
        case 'delete_ticket': {
            const issueIdOrKey = request.params.arguments?.issueIdOrKey;
            if (!issueIdOrKey) {
        ...
        case 'edit_ticket':
            ...
            const summary = request.params.arguments?.summary;
            const description = request.params.arguments?.description;
            const labels = request.params.arguments?.labels;
            const parent = request.params.arguments?.parent;
            if (!issueIdOrKey) {
                throw new Error('Issue id or key is required');
            }
        ...
        case 'assign_ticket': {
            const accountId = request.params.arguments?.accountId;
            const issueIdOrKey = request.params.arguments?.issueIdOrKey;
        ...
        case 'query_assignable': {
            const project_key = request.params.arguments?.project_key;
            if (!project_key) {
        ...
        case 'add_attachment_from_confluence': {
            const issueIdOrKey = request.params.arguments?.issueIdOrKey;
            const pageId = request.params.arguments?.pageId;
            const attachmentName = request.params.arguments?.attachmentName;
